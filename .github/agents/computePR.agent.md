...
6) **Tests**  
   ...
   - **Every affected cmdlet** — if the feature touches multiple cmdlets (e.g., a `New-Az*` and its matching `Update-Az*`), cover the scenarios on each cmdlet, not just one. For `Get-*` output changes, assert the new property round-trips.
   ...
   - **Negative & edge cases** — mutually exclusive parameters used together should error (use `Assert-ThrowsContains` / `Assert-Throws`); passing an explicit `$null` or empty array to an array parameter should not throw; absence of the parameter should leave behavior unchanged.
   ...
   - Always round-trip through `Get-*` after the mutation and re-assert, to confirm the state persisted server-side (not just echoed back by the mutating call).
...
- Test assertions verify concrete sub-properties (enum/type value, collection counts and membership, computed fields) and round-trip through `Get-*`, not just that a non-null object was returned.
...
- [ ] Test assertions check concrete sub-properties (type/enum, collection counts and membership, computed fields) and round-trip through `Get-*`.
- [ ] Every new `.ps1` scenario function is wired into the `.cs` file as a `[Fact]` check-in test.
