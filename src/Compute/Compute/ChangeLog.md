...
-->
## Upcoming Release
* Added `-SystemAssignedIdentity` and `-UserAssignedIdentity` parameters to `New-AzGallery` and `Update-AzGallery` cmdlets to support Managed Identity configuration on Azure Compute Galleries.
...
* Added support for assigning multiple user assigned managed identities to a gallery using the `-UserAssignedIdentity` parameter, which accepts one or more ARM (Azure Resource Manager) resource IDs (for example, the `Id` property returned by `Get-AzUserAssignedIdentity`).
